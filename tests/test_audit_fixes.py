...
def test_apply_ocr_result_composes_only_active_regions(monkeypatch):
    """End-to-end through the result applier: the utterance handed to TTS must
    not contain a background process's text."""
    spoken = []
    tts = SimpleNamespace(
    ...
    def poll():
        # Fresh result each time: the applier consumes has_pending_frame.
    ...
    poll()          # first sighting -> candidate (jitter guard)
    fg.last_text = ""    # force a "changed" second poll with the same text
    poll()          # confirmation -> speaks
    assert spoken, "expected an utterance"
    assert "Background chatter." not in spoken[-1]
    assert "Foreground line." in spoken[-1]
